                                            ...
                                            <div>
                                                ...
                                                {block.items && block.items.length > 0 && (
                                                    <div className={`grid grid-cols-1 ${block.imageUrl ? 'md:grid-cols-2' : 'md:grid-cols-2 lg:grid-cols-3'} gap-6 lg:gap-8`}>
                                                        {block.items?.map((s, i) => {
                                                            const cardClasses = "group bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/10 p-6 md:p-8 rounded-3xl shadow-sm hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300 hover:-translate-y-2 relative overflow-hidden h-full flex flex-col block";
                                                            const CardInner = (
                                                                <>
                                                                    {s.backgroundImage && (
                                                                    ...
                                                                    <div className="relative z-10 flex flex-col flex-1">
                                                                        {s.icon && <div className={`text-4xl mb-6 ${s.backgroundImage ? 'bg-white/20' : 'bg-blue-50 dark:bg-blue-500/10'} w-16 h-16 rounded-2xl flex items-center justify-center border ${s.backgroundImage ? 'border-white/30 text-white' : 'border-blue-100 dark:border-blue-500/20'}`}>{s.icon}</div>}
                                                                        <h5 style={{ ...getSubHeadingStyle(block).style, fontSize: (block.subHeadingSize && block.subHeadingSize !== 'default') ? block.subHeadingSize : undefined }} className={`font-bold text-xl mb-4 ${s.backgroundImage ? 'text-white' : getSubHeadingStyle(block).className || "text-slate-900 dark:text-white"}`}>{s.title}</h5>
                                                                        <p style={{ ...getTextStyle(block).style, fontSize: (block.bodyTextSize && block.bodyTextSize !== 'default') ? block.bodyTextSize : undefined }} className={`leading-relaxed text-sm md:text-base ${s.backgroundImage ? 'text-white/90' : getTextStyle(block).className || "text-slate-600 dark:text-slate-400"} flex-1`}>{s.desc}</p>
                                                                    </div>
                                                                </>
                                                            );
                                            ...
                                            <LayoutWrapper>
                                                <div>
                                                    ...
                                                    {block.steps && block.steps.length > 0 && (
                                                        <div className={`space-y-8 relative before:absolute before:inset-0 before:ml-6 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-blue-200 dark:before:via-blue-900 before:to-transparent ${hasSideImage ? '' : 'md:before:mx-auto md:before:translate-x-0'}`}>
                                                            {block.steps?.map((step, i) => (
                                                                <div key={i} className={`relative flex items-center justify-between group ${hasSideImage ? 'md:justify-start gap-6' : 'md:justify-normal md:odd:flex-row-reverse'}`}>
                                                                    ...
                                                                    <div className={`w-[calc(100%-4rem)] bg-white dark:bg-slate-900/60 p-6 md:p-8 rounded-3xl border border-slate-200 dark:border-white/5 shadow-sm hover:shadow-xl hover:border-blue-300 dark:hover:border-blue-500/30 transition-all duration-300 min-w-[240px] sm:min-w-[280px] ${hasSideImage ? '' : 'md:w-[calc(50%-3rem)]'}`}>
